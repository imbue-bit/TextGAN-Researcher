...
class KnowledgeFreshnessAnalysisTool(BaseTool):
    """知识时效性分析工具"""
    name: str = "analyze_knowledge_freshness"
    descriptio: str = "Analyze the freshness and relevance of collected knowledge items within a specified time period. Input: time_period (e.g., '3 个月', '1 年'). Output: A report on knowledge freshness. Example input: '6 个月'"
    
    def __init__(self, state: EnhancedExecutionState):
        super().__init__()
        self.state = state
    
    def _run(self, time_period: str = "1 年") -> str:
        if "天" in time_period:
        ...
        age_buckets = {"1 个月内": 0, "1-3 个月": 0, "3-6 个月": 0, "6 个月 -1 年": 0, "1 年以上": 0}
            ...
            try:
                if item.publish_date:
                    ...
                    if age.years == 0 and age.months == 0 and age.days < 30:
                        ...
                        age_buckets["1-3 个月"] += 1
                    elif age.years == 0 and age.months < 6:
                        age_buckets["3-6 个月"] += 1
                    elif age.years == 0:
                        age_buckets["6 个月 -1 年"] += 1
                    else:
                        age_buckets["1 年以上"] += 1
                else:
                    age_buckets["1 年以上"] += 1  # 没有发布日期，视为旧数据
            except:
                age_buckets["1 年以上"] += 1
...
class ContradictionDetectionTool(BaseTool):
    """矛盾检测工具"""
    name: str = "detect_contradictions"
    description: str = "Analyze the knowledge base for conflicting information. Input: A JSON list of knowledge item keys to check (optional, checks all if empty). Output: A report on detected contradictions. Example input: '[\"AI_benefits\", \"AI_risks\"]'"
    ...
    def __init__(self, llm: BaseChatModel, state: EnhancedExecutionState):
        ...
        self.prompt = PromptTemplate.from_template(
            ...
            请分析这些知识条目。如果发现矛盾，请描述矛盾点，并列出涉及矛盾的知识条目 key。
            如果不存在明显矛盾，请说明。
...
class HypothesisGenerationTool(BaseTool):
    ...
    description: str = "Generate a new research hypothesis or question based on the current knowledge base. Input: Optional string describing the area of focus. Output: The generated hypothesis string. Example input: 'impact of climate change on specific crops'"
...
class VerifyHypothesisTool(BaseTool):
    """验证假设工具"""
    name: str = "verify_hypothesis"
    description: str = "Attempt to verify or refute a specific hypothesis by searching for supporting or contradicting evidence. Input: 'hypothesis_id' (string). Output: A summary of the verification attempt and updated hypothesis status. Example input: 'hyp_1'"
    ...
    def _run(self, hypothesis_id: str) -> str:
        try:
            ...
            if not hypothesis:
                return f"未找到假设 ID: {hypothesis_id}"
            
            # 1. 自动进行相关搜索 (Agent 内部调用工具)
            search_query = f"evidence for/against '{hypothesis['hypothesis']}'"
            search_results_json = self.web_search_tool._run(search_query)  # 直接调用工具的_run 方法

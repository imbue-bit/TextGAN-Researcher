...
class InformationExtractionTool(BaseTool):
    ...
    def __init__(self, llm: BaseChatModel, state: EnhancedExecutionState):
        ...
        self.prompt = PromptTemplate.from_template(
            """你是一个专业的信息提取专家。你的任务是从提供的文本中提取关键信息，并将其结构化为KnowledgeItem。
            ...
            请提取最重要的事实、观点或数据，并以JSON格式的KnowledgeItem返回。
            KnowledgeItem 结构: {{"key": "主题/名称", "value": "提取的具体内容", "source": "来源URL或描述", "publish_date": "ISO格式日期 (如果文本中包含)", "confidence": "0.0-1.0", "goal_id": "关联的子目标ID"}}
            
            如果无法确定发布日期，请省略publish_date字段。如果文本中没有明确的可信度信息，请使用默认值0.7。
            仅返回JSON，不要有其他说明。"""
        )
...
class ViewKnowledgeBaseTool(BaseTool):
    ...
    description = "View a summary of the current knowledge base. Input: Optional JSON string with 'limit' (int) for number of items, 'recency_days' (int) for recent items. Output: A summary string of knowledge items. Example input: '{{\"limit\": 5, \"recency_days\": 30}}'"
